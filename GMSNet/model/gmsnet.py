...
class GMSNet(nn.Module):
    def __init__(self):
        ...
        self.inc = nn.Sequential(
            nn.Conv2d(3, self.planes, kernel_size=3, padding=1), nn.PReLU()
        )
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(
                    m.weight,
                    mean=0,
                    std=np.sqrt(2 / (m.weight.shape[0] * np.prod(m.weight.shape[2:])))
                    * reduce(lambda x, y: x * y, self.num_groups) ** (-0.5),
                )
